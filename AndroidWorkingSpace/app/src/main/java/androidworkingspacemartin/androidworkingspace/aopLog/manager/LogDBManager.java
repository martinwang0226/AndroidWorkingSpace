...
import androidworkingspacemartin.androidworkingspace.aopLog.model.LogState;
...
public class LogDBManager implements ILogStore {
    ...
    public synchronized boolean storeLog(LinkedList<MsgModel> list, int count) {
        ...
        try {
            ...
            Log.i("chen","异常数据库文件的地址："+filePath);
     ...
     * 检查数据库，删除过期的数据
     *
     * @param logFile 数据库的地址
     */
    public synchronized void checkLog(File logFile) {
    ...
    private synchronized void checkLogBySize(LogState logState){
        ...
        Log.i("chen","需要检查的文件的路径是："+logFile.getAbsolutePath());
        try{
            //判断文件的大小是否在规定范围之内
            if (logFile != null && logFile.exists() && logFile.length() > LogService.MAX_SIZE_FILE) {
                ...
                Log.i("chen","压缩前文件的大小是："+logFile.length());
                ...
                Log.i("chen","压缩后的文件的地址："+zipFilePath);
                db = mHelper.getWritableDatabase();
                db.beginTransaction();
                if (TextUtils.isEmpty(zipFilePath)) {
                    //压缩文件失败：删除一半的数据，然后添加 一条标识的数据
                    Log.i("chen","minId:"+ logState.getMinId()+",maxId:"+ logState.getMaxId());
                    //删除数据库一半的数据
